	...
	SubShader
	{
		...
		Pass
		{
			...
			float3 calculateWorldSpace(float4 vertex, float2 screenOffset)
			{
				...
				// clip(frac((worldPos.y)*displacement) - _SliceScale);
			...
			{
				float displacement = 0.5  * _Scale * sin(_Time.y * _Frequency) + 1;
				clip(frac((i.modelPos.y)*displacement) - _SliceScale);
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				if(unity_StereoEyeIndex  == 0){
					grabPos.x += ( rand( float2( grabPos.y, _Time.y ) ) - 0.5 )* 0.005;
					grabPos.x += ( rand( float2( grabPos.y * 100.0, _Time.y * 10.0 ) ) - 0.5 ) * 0.01;
					// col = tex2D(_WorldSpaceNormal, grabPos).rgb;
					//
					// float2 redShift = float2(grabPos.x + rgbShift(), grabPos.y);
					// float2 blueShift = float2(grabPos.x, grabPos.y + rgbShift());
					// col.r = tex2D(_WorldSpaceNormal, redShift).r;
					// col.g = tex2D(_WorldSpaceNormal, grabPos).g;
					// col.b = tex2D(_WorldSpaceNormal, blueShift).b;
				...
				float4 col = tex2D(_WorldSpaceNormal, grabPos);
			// 	float2 offset = 1.0 / _ScreenParams.xy;
			// //	float3 col = tex2D(_WorldSpaceNormal, grabPos).rgb;
			...
			// 	float w = dot(one, abs(pos10 - col)) + dot(one, abs(pos01 - col));
			// 	float4 wColor = _WireframeColor;
			// 	wColor.r += rgbShift(); wColor.b += rgbShift();

				//return lerp(tex2D(_SceneBefore, grabPos), wColor, w * _WireframeColor.a);
				//return fixed4(col, 1.0);
				return lerp(tex2D(_SceneBefore, grabPos), col , 0.0);
